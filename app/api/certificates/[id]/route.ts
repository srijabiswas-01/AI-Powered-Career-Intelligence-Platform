...
export async function DELETE(_:Request,{params}:{params:Promise<{id:string}>}){const user=await getSessionUser();if(!user)return apiError('Authentication required.',401);const {id}=await params;const rows=await database`delete from certificates where id=${id} and user_id=${user.id} returning id`;return rows.length?NextResponse.json({ok:true}):apiError('Certificate not found.',404)}
