...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (resume.file_data) body = new Uint8Array(resume.file_data);
    ...
    const source = await fetch(resume.storage_url, { cache: 'no-store' });
    if (!source.ok || !source.body) return apiError('This older PDF is blocked by Cloudinary. Delete and re-upload it once to enable reliable viewing.', 409);
    body = source.body;
  } else return apiError('Resume file not found.', 404);
  const encoded = encodeURIComponent(resume.filename).replace(/['()]/g, escape);
  return new Response(body, {
    headers: {
      'Content-Type': resume.mime_type || (resume.filename.toLowerCase().endsWith('.pdf') ? 'application/pdf' : 'application/octet-stream'),
